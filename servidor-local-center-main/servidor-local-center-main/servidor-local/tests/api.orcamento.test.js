...
describe('Orcamento API Deep Dive', () => {
    ...
    beforeEach(() => {
        ...
        Object.values(OrcamentoModel).forEach((m) => m?.mockReset?.());
    });
    ...
    it('PUT /calcular/:id should calculate budget with mocked data', async () => {
        ...
        const { OrcamentoModel } = require('../src/models/orcamento.models.js');
        PrestacaoServicoModel.getByOrcamentoId.mockResolvedValue([
            { horas_estimadas: 2, preco_hora: 50, id_prestador: 'p1', id_orcamento: '1', urgente: true }
        ]);
        PrestadorModel.get.mockResolvedValue({ taxa_urgencia: 10, minimo_desconto: 20, percentagem_desconto: 5 });
        OrcamentoModel.atualizarTotal.mockResolvedValue(true);
        const app = await appPromise();
...
//# sourceMappingURL=api.orcamento.test.js.map